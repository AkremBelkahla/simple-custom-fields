...
class SCF_Admin_Page {
    ...
    public function save_field_group() {
        ...
        $status = isset($_POST['status']) && $_POST['status'] == '1' ? 'publish' : 'draft';
    ...
    private function sanitize_rules($rules) {
        ...
        return array(
            'param' => isset($rules['param']) ? sanitize_key($rules['param']) : 'post_type',
            'operator' => isset($rules['operator']) && in_array($rules['operator'], array('=', '!=')) ? $rules['operator'] : '=',
            'value' => isset($rules['value']) ? sanitize_key($rules['value']) : 'post'
        );
    }
